...
function goToCheckout() {
    ...
    // Use Shopify's cart API to add items, then redirect to checkout
    fetch('https://91teer-01.myshopify.com/cart/add.js', {
        method: 'POST',
        headers: {
    ...
    .then(() => {
        // Redirect to checkout after items are added
        window.location.href = 'https://91teer-01.myshopify.com/checkout';
    })
    .catch((error) => {
        console.error('Error adding to cart:', error);
        // Fallback: try direct cart link
        let cartItems = [];
        cart.forEach(item => {
            const variantId = variantIds[item.name][item.size];
            const quantity = item.quantity || 1;
        ...
        window.location.href = `https://91teer-01.myshopify.com/cart/${cartItems.join(',')}`;
    });
}
}
